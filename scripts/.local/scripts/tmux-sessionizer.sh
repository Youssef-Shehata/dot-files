...
if (( ${#TS_SEARCH_PATHS[@]} == 0 )); then
    TS_SEARCH_PATHS=("$HOME/Work" "$HOME/Practice/" "$HOME/dot-files/")
fi
...
find_dirs() {
    ...
    for entry in "${TS_SEARCH_PATHS[@]}"; do
        ...
        [[ -d "$path" ]] && LC_ALL=C find "$path" -mindepth 1 -maxdepth "${depth:-${TS_MAX_DEPTH:-3}}"  \( -path '*/.git' -o -path '*/node_modules' \) -prune -o -type d -print
    done
}
